...
def calculate_health_score(financials, ticker):
    ...
    # Adjusted scoring for Utilities Sector characteristics
    rev_score = min(max((revenue / 1e9) * 1.5, 0), 15) if revenue else 0
    margin_score = min(max(net_margin * 2.0, 0), 25) if net_margin else 0
    debt_score = min(20 - min(debt_to_equity * 1.5, 20), 20) if debt_to_equity else 0
    roe_score = min(roe * 0.6, 25) if roe else 0

    total_score = rev_score + margin_score + debt_score + roe_score
...
# Run analysis on your list of utility tickers
analyze_stocks("my_utilities.txt", "utility_stock_health_report.csv")
